...
def clean_text(html: str) -> str:
    ...
    text = re.sub(r'[\r\t]+', ' ', text)  # replace newlines/tabs with space
    text = re.sub(r'\s{2,}', ' ', text)  # collapse multiple spaces
    text = re.sub(r"Unity - Manual: .+ Manual Scripting API unity\.com Version: Unity 6\.4 \(6000\.4\) Supported Legacy Language : English English 中文 日本語 한국어 Unity Manual Version: Unity 6\.4 Select a different version Language : English English 中文 日本語 한국어", '', text)
    text = text.strip()
...
url_loader = RecursiveUrlLoader(
    ...
    max_depth=2,
    extractor=clean_text,  # 👈 applied to each page before chunking
)
url = url_loader.load()
